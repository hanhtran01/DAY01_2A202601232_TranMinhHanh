...
def compare_models(prompt: str) -> dict:
    ...
    gpt4o_answer, gpt4o_time = call_openai(
        prompt,
        model=OPENAI_MODEL
    )

    mini_answer, mini_time = call_openai_mini(
        prompt
    )
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Giữ tối đa 4 lượt hội thoại
        # 1 lượt = user + assistant = 2 message
        history = history[-8:]
...
def retry_with_backoff(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # Nếu đã hết số lần thử thì ném lỗi cuối cùng
            if attempt == max_retries:
                raise last_exception

            # exponential backoff:
            # lần 1: base_delay * 2^0
            # lần 2: base_delay * 2^1
            # lần 3: base_delay * 2^2
...
def run_assistant(
    ...
    while True:
        ...
        # Thống kê token
        prompt_tokens = count_tokens(user_msg, OPENAI_MODEL)
        completion_tokens = count_tokens(reply, OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for item in results:
        rows.append(
            " | ".join(
                [
                    shorten(item.get("prompt", "")),
                    shorten(item.get("gpt4o_answer", "")),
                    shorten(item.get("mini_answer", "")),
                    f'{item.get("gpt4o_time", 0):.2f}s',
                    f'{item.get("mini_time", 0):.2f}s',
                ]
            )
        )
